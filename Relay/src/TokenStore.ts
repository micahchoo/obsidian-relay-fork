...
export class TokenStore<TokenType extends HasToken> {
	...
	onRefresh(documentId: string): Promise<TokenType> {
		console.debug("[TokenStore.onRefresh] entered, will call refresh()", { documentId });
		const promise = new Promise((resolve, reject) => {
			const onSuccess = (token: TokenType) => {
				console.debug("[TokenStore.onRefresh] onSuccess", { documentId, hasToken: !!(token as HasToken)?.token });
				resolve(token);
			};
			const onError = (error: Error) => {
				console.debug("[TokenStore.onRefresh] onError", { documentId, error: error?.message });
	...
	private getTokenFromNetwork(
		...
		if (activePromise) {
			console.debug("[TokenStore.getTokenFromNetwork] returning EXISTING activePromise (possibly stuck)", {
				documentId,
				friendlyName,
			});
			return activePromise;
		}
		console.debug("[TokenStore.getTokenFromNetwork] new request", { documentId, friendlyName });
		const existing = this.tokenMap.get(documentId);
		this.tokenMap.set(documentId, {
			token: null,
	...
	async getToken(
		...
		console.debug("[TokenStore.getToken] entered", {
			documentId,
			friendlyName,
			hasActivePromise: this._activePromises.has(documentId),
			hasCachedToken: this.tokenMap?.has(documentId),
		});
