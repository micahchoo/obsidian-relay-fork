...
async function refresh(
	...
	console.debug("[LiveTokenStore.refresh] entered", { documentId, loggedIn: loginManager.loggedIn });
	debug(`${documentId}`);
	...
	try {
		...
		console.debug("[LiveTokenStore.refresh] POST /token", { apiUrl, payload });
		const response = await customFetch(`${apiUrl}/token`, {
			method: "POST",
			headers: headers,
			body: payload,
		});
		console.debug("[LiveTokenStore.refresh] got response", { status: response.status, ok: response.ok });

		if (!response.ok) {
			const body = await response.text();
			console.warn("[LiveTokenStore.refresh] non-OK", { status: response.status, body });
		...
		console.debug("[LiveTokenStore.refresh] parsed token", { hasToken: !!clientToken?.token, url: clientToken?.url });
		onSuccess(clientToken);
	} catch (reason) {
		console.warn("[LiveTokenStore.refresh] fetch threw", reason);
