...
export class HasProvider extends HasLogging {
	...
	connect(): Promise<boolean> {
		console.debug("[HasProvider.connect] entered", {
			guid: this.guid,
			path: this.path,
		...
		return this.getProviderToken()
			.then((clientToken) => {
				console.debug("[HasProvider.connect] got token, calling provider.connect()", {
					guid: this.guid,
					hasToken: !!clientToken?.token,
				});
			...
			.catch((e) => {
				this.warn(`[${this.path}] connect() failed:`, e?.message || e);
				return false;
			});
	}
