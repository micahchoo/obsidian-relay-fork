...
export class SharedFolder extends HasProvider {
	...
	connect(): Promise<boolean> {
		const isRemote = this.s3rn instanceof S3RemoteFolder;
		console.debug("[SharedFolder.connect] entered", {
			guid: this.guid,
			path: this.path,
			isRemote,
			connected: this.connected,
			shouldConnect: this.shouldConnect,
			s3rnType: this.s3rn?.constructor?.name,
		});
		if (isRemote) {
			if (this.connected || this.shouldConnect) {
				return super.connect();
			}
			console.debug("[SharedFolder.connect] skipped: !connected && !shouldConnect", {
				guid: this.guid,
			});
		} else {
			console.debug("[SharedFolder.connect] skipped: not S3RemoteFolder", {
				guid: this.guid,
				s3rnType: this.s3rn?.constructor?.name,
			});
		}
